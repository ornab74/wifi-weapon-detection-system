#!/usr/bin/env python3
"""
main.py — WiFi-DensePose fork advanced CSI anomaly head, v0.3
...
def detect_from_files(
    ...
    *,
    ...
    empty = load_csi_npz(Path(empty_path))
    scan = load_csi_npz(Path(scan_path))
    return detect(empty, scan, cfg)
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "Advanced research detector for barrel-like long conductive-object "
            "anomalies inside bags using WiFi CSI. Does not confirm weapons."
        )
    )
    ...
    p.add_argument("--threshold", type=float, default=0.85, help="Alert threshold in [0, 1].")
    p.add_argument("--quality-threshold", type=float, default=0.35, help="Minimum sensor quality in [0, 1].")
    p.add_argument("--min-frames", type=int, default=8, help="Minimum frames required in each CSI file.")
    p.add_argument("--restricted-zone-prior", type=float, default=0.50, help="Context prior in [0,1].")
    p.add_argument("--bag-transition-prior", type=float, default=0.50, help="Context prior in [0,1].")
    p.add_argument("--carrier-hz", type=float, default=5.8e9, help="RF carrier frequency for wavelength/Doppler proxies.")
    p.add_argument("--bandwidth-hz", type=float, default=80e6, help="Bandwidth for range-resolution proxy.")
    p.add_argument("--link-distance-m", type=float, default=3.0, help="Link distance for Fresnel radius proxy.")
    p.add_argument("--sample-rate-hz", type=float, default=100.0, help="CSI frame/sample rate for Doppler proxy.")
    ...
    p.add_argument("--make-demo", action="store_true", help="Generate synthetic demo CSI files and exit.")
    p.add_argument("--demo-dir", type=Path, default=Path("demo_csi"), help="Directory for --make-demo files.")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        ...
        if args.empty is None or args.scan is None:
            if args.equations_out is not None:
                print(json.dumps({"equations_out": str(args.equations_out), "claim_boundary": CLAIM_BOUNDARY}, indent=2))
                return 0
            raise CSIInputError("Provide --empty and --scan, or use --make-demo.")
        ...
        result = detect_from_files(
            ...
            restricted_zone_prior=float(args.restricted_zone_prior),
            bag_transition_prior=float(args.bag_transition_prior),
            carrier_hz=float(args.carrier_hz),
            bandwidth_hz=float(args.bandwidth_hz),
            link_distance_m=float(args.link_distance_m),
